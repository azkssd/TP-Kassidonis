...
object Main {
  def main(args: Array[String]) {
    ...
    val climateVsNonClimateNewsCount = enrichedDataset.groupBy($"containsWordGlobalWarming").count()
    climateVsNonClimateNewsCount.show()
    ...
    spark.sql("SELECT * FROM news").show()
    ...
    // enrichedDataset.write.partitionBy("date", "media").parquet(...)
